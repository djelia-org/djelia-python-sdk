...
from djelia.config.settings import BASE_URL, ENV_API_KEY, API_KEY_HEADER
...
class DjeliaRequest:
    ...
    get_supported_languages: HttpRequestInfo = HttpRequestInfo(
        endpoint=endpoint_prefix + "translate/supported-languages",
        method="GET"
    )
    translate: HttpRequestInfo = HttpRequestInfo(
        endpoint=endpoint_prefix + "translate",
        method="POST"
    )    

    transcribe: HttpRequestInfo = HttpRequestInfo(
        endpoint=endpoint_prefix + "transcribe",
        method="POST"
    )

    transcribe_stream: HttpRequestInfo = HttpRequestInfo(
        endpoint=endpoint_prefix + "transcribe/stream",
        method="POST"
    )    

    tts: HttpRequestInfo = HttpRequestInfo(
        endpoint=endpoint_prefix + "tts",
        method="POST"
    )

    tts_stream: HttpRequestInfo = HttpRequestInfo(
        endpoint=endpoint_prefix + "tts/stream",
        method="POST"
    )
...
class ErrorsMessage:
    ...
    speaker_description_error: str = "Description must contain one of the supported speakers: {}"
    speaker_id_error: str = "Speaker ID must be one of {}, got {}"
    api_key_missing: str = f"API key must be provided via parameter or {ENV_API_KEY} environment variable"  
