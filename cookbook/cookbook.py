...
class DjeliaCookbook:
    ...
    def test_translation_sync(self) -> None:
        ...
        try:
            languages: List[SupportedLanguageSchema] = self.sync_client.translation.get_supported_languages()
            ...
            self.test_results[test_name] = ("Success", f"{len(self.translation_samples)} translations")
    ...
    async def test_translation_async(self) -> None:
        ...
        async with self.async_client as client:
            try:
                ...
                for text, source, target in self.translation_samples:
                    request = TranslationRequest(text=text, source=source, target=target)
                    response = await client.translation.translate(request=request, version=Versions.v1)
                    print_success(
            ...
            try:
                ...
                if version == Versions.v2:
                    ...
                    transcription_fr = self.sync_client.transcription.transcribe(
                        self.config.audio_file_path, translate_to_french=True, version=version
                    )
                    print_success(
                ...
                self.test_results[test_name] = ("Success", f"v{version.value} completed")
        ...
        async with self.async_client as client:
            for version in [Versions.v1, Versions.v2]:
                try:
                    ...
                    handle_transcription_result(transcription, f"v{version.value} (async)")
            ...
            try:
                ...
                for segment in self.sync_client.transcription.transcribe(
                    ...
                    if segment_count >= self.config.max_stream_segments:
                        print_info(f"Showing first {self.config.max_stream_segments} segments")
                        break
                ...
                if version == Versions.v2:
                    ...
                    for segment in self.sync_client.transcription.transcribe(
                        self.config.audio_file_path, stream=True, translate_to_french=True, version=version
                    ):
                        segment_count += 1
                        text = segment.text
                        print_success(
                    ...
                    print_success(f"French streaming complete: {segment_count} segments")
    ...
    def test_tts_sync(self) -> None:
        ...
        try:
            ...
            audio_file_v1 = self.sync_client.tts.text_to_speech(
                request=tts_request_v1, output_file=f"tts_sync_v1_{uuid4().hex}.wav", version=Versions.v1
            )
            print_success(f"TTS v1 saved: {ConsoleColor.BLUE}{audio_file_v1}{ConsoleColor.RESET}")
            ...
            for speaker in self.supported_speakers:
                ...
                print_success(f"TTS v2 ({speaker}): {ConsoleColor.BLUE}{audio_file_v2}{ConsoleColor.RESET}")
                
            self.test_results[test_name] = ("Success", f"{len(self.supported_speakers)} speakers")
        ...
        async with self.async_client as client:
            try:
                ...
                audio_file_v1 = await client.tts.text_to_speech(
                    request=tts_request_v1, output_file=f"tts_async_v1_{uuid4().hex}.wav", version=Versions.v1
                )
                print_success(f"TTS v1 saved: {ConsoleColor.BLUE}{audio_file_v1}{ConsoleColor.RESET}")
    ...
    def test_streaming_tts_sync(self) -> None:
        ...
        try:
            ...
            for audio_chunk in self.sync_client.tts.text_to_speech(
                request=tts_request, output_file=f"stream_tts_sync_{uuid4().hex}.wav", stream=True, version=Versions.v2
            ):
                chunk_count += 1
        ...
        async with self.async_client as client:
            try:
                ...
                generator = await client.tts.text_to_speech(
                    request=tts_request, output_file=f"stream_tts_async_{uuid4().hex}.wav", stream=True, version=Versions.v2
                )
        ...
        async with self.async_client as client:
            try:
                ...
                results = await asyncio.gather(
                    ...
                    client.transcription.transcribe(self.config.audio_file_path, version=Versions.v1),
                    client.tts.text_to_speech(
                        tts_request, output_file=f"parallel_tts_{uuid4().hex}.wav", version=Versions.v2
                    ),
                ...
                if all(not isinstance(r, Exception) for r in results):
                    self.test_results[test_name] = ("Success", "All operations completed")
                else:
                    failed = [type(r).__name__ for r in results if isinstance(r, Exception)]
                    self.test_results[test_name] = ("Failed", f"Errors: {', '.join(failed)}")
    ...
    def test_version_management(self) -> None:
        ...
        print_success(f"Latest version: {ConsoleColor.YELLOW}{Versions.latest()}{ConsoleColor.RESET}")
        print_success(f"Available versions: {ConsoleColor.GRAY}{[str(v) for v in Versions.all_versions()]}")
        
        try:
            request = TranslationRequest(text="Hello world", source=Language.ENGLISH, target=Language.BAMBARA)
            result = self.sync_client.translation.translate(request=request, version=Versions.v1)
            print_success(f"Translation v1: '{ConsoleColor.YELLOW}{result.text}{ConsoleColor.RESET}'")
